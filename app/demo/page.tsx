...
type Section = 'dashboard' | 'contacts' | 'pipeline' | 'conversations' | 'study' | 'mailing' | 'agent'
...
function normalizeContact(contact: Contact): Contact { return { ...contact, company: contact.company || contact.name, city: contact.city || 'Milano', priorityLevel: contact.priorityLevel || 'B', outreachStage: contact.outreachStage || 'Da qualificare', generalEmail: contact.generalEmail || contact.email || '', sourceBatch: contact.sourceBatch || '', preferredEnergyPath: contact.preferredEnergyPath || 'Da valutare' } }
...
export default function DemoAppPage() {
  ...
  function buildWhatsAppTemplate(contact: Contact): string {
    ...
    if (assistedTemplate === 'tailored') return `Buongiorno ${person}, prima di farle un preventivo standard preferirei capire la situazione reale di ${business}: costi attuali, problemi principali, valore aziendale indicativo e possibile plafond servizi. Oggi pesa di più ridurre costi, gestire richieste/prenotazioni, alleggerire lo staff o valutare energia/bollette?`
  ...
  const nav = [{ id: 'dashboard', label: 'Dashboard', icon: TrendingUp }, { id: 'contacts', label: 'Database 100', icon: Users }, { id: 'pipeline', label: 'Flusso', icon: ChevronRight }, { id: 'conversations', label: 'Comunicazioni', icon: Upload }, { id: 'study', label: 'Studio su misura', icon: Calculator }, { id: 'mailing', label: 'Mailing CCN', icon: Mail }, { id: 'agent', label: 'Agente', icon: Bot }] as const
