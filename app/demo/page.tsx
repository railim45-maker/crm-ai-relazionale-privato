...
const networkMaterials: NetworkMaterial[] = [
  {
    ...
    assets: [{ label: 'Sito/demo VoiceDesk', href: 'https://www.voicedesk.it/' }, { label: 'Messaggio WhatsApp' }, { label: 'Scheda settore da preparare sul lead' }],
...
function voiceDeskProfile(contact?: Partial<Contact> | null) {
  ...
  if (segment === 'Hotel') return { segment, label: 'Hotel', phone: voiceDeskHotelPhone, displayPhone: voiceDeskHotelPhoneDisplay, context: 'reception e richieste ospiti' }
  if (segment === 'Ristorante') return { segment, label: 'Ristorante', phone: voiceDeskRestaurantPhone, displayPhone: voiceDeskRestaurantPhoneDisplay, context: 'prenotazioni e richieste tavoli' }
  return { segment: 'Generico' as const, label: 'Categoria da confermare', phone: voiceDeskRestaurantPhone, displayPhone: voiceDeskRestaurantPhoneDisplay, context: 'prime chiamate e richieste clienti' }
}
...
function buildVoiceDeskEmailTemplate(contact: Contact) {
  ...
  const callLink = buildTrackedVoiceDeskUrl(contact, 'call', `tel:${profile.phone}`)
  const openPixel = buildTrackedVoiceDeskUrl(contact, 'open', '')
  const subject = profile.segment === 'Hotel' ? 'La demo per la reception' : profile.segment === 'Ristorante' ? 'La demo per le prenotazioni' : 'Una demo da provare'
  const person = (contact.decisionMakerName || contact.name || 'buongiorno').trim()
  const body = `Buongiorno ${person},\n\nle lascio una prova rapida di VoiceDesk per ${profile.context}.\n\nDemo: ${demoLink}\nChiamata diretta: ${callLink}\n\nA presto.`
  const html = `<p>Buongiorno ${person},</p><p>le lascio una prova rapida di <strong>VoiceDesk</strong> per ${profile.context}.</p><p><a href="${callLink}" style="display:inline-block;background:#111827;color:#ffffff;padding:12px 18px;border-radius:12px;text-decoration:none;font-weight:700">Prova la chiamata demo</a></p><p><a href="${demoLink}">Apri la demo VoiceDesk</a></p><img src="${openPixel}" width="1" height="1" alt="" style="display:none" />`
