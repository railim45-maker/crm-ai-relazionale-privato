/*
 * FILE GENERATO DA MANUS - PAGINA DEMO - CRM dimostrativo con campi studio etichettati
...
type Section = 'dashboard' | 'contacts' | 'calendar' | 'pipeline' | 'conversations' | 'research' | 'study' | 'team' | 'investors' | 'documents' | 'materials' | 'mailing' | 'agent'
...
const networkMaterials: NetworkMaterial[] = [
  {
    ...
    assets: [{ label: 'Sito/demo VoiceDesk', href: 'https://www.voicedesk.it/' }, { label: 'Video demo hotel', href: voiceDeskHotelVideoUrl, hotelOnly: true }, { label: 'Messaggio WhatsApp' }, { label: 'Scheda settore da preparare sul lead' }],
...
function detectVoiceDeskSegment(contact?: Partial<Contact> | null): 'Ristorante' | 'Hotel' | 'Generico' {
  ...
  if (/hotel|albergo|b&b|bed and breakfast|ospitalit|reception|camere|booking/.test(haystack)) return 'Hotel'
...
function voiceDeskProfile(contact?: Partial<Contact> | null) {
  ...
  if (segment === 'Hotel') return { segment, label: 'Hotel / struttura ricettiva', phone: voiceDeskHotelPhone, displayPhone: voiceDeskHotelPhoneDisplay, context: 'reception, richieste ospiti e prenotazioni' }
...
function voiceDeskHotelVideoLine() {
  return `[video allegato: ${voiceDeskHotelVideoUrl}]`
}
...
function buildVoiceDeskSocialTemplate(contact: Contact, channel: Channel = 'Instagram') {
  ...
  if (profile.segment === 'Hotel') return `Hai visto cosa puoi fare nel tuo hotel?
...
function buildVoiceDeskEmailTemplate(contact: Contact) {
  ...
  const subject = profile.segment === 'Hotel' ? 'Hai visto cosa fa VoiceDesk nel tuo hotel?' : profile.segment === 'Ristorante' ? 'Demo prenotazioni pronta' : 'Demo VoiceDesk pronta'
  const person = (contact.decisionMakerName || contact.name || 'buongiorno').trim()
  const hotelBody = `Buongiorno ${person},

Hai visto cosa può fare VoiceDesk nel tuo hotel?

Guarda il video demo allegato e, se vuoi provarlo subito, puoi chiamare direttamente la demo hotel.
...
A presto.`
  ...
  const html = profile.segment === 'Hotel'
    ? `<p>Buongiorno ${person},</p><p><strong>Hai visto cosa può fare VoiceDesk nel tuo hotel?</strong></p><p>Guarda il video demo allegato e, se vuoi provarlo subito, puoi chiamare direttamente la demo hotel.</p><p><a href="${callLink}" style="display:inline-block;background:#111827;color:#ffffff;padding:12px 18px;border-radius:12px;text-decoration:none;font-weight:700">Chiama la demo VoiceDesk</a></p><p style="font-size:14px;color:#374151">Numero diretto: <strong>${profile.displayPhone}</strong></p><p><a href="${voiceDeskHotelVideoUrl}">Apri video demo hotel</a></p><p><a href="${demoLink}">Apri la landing VoiceDesk</a></p><img src="${openPixel}" width="1" height="1" alt="" style="display:none" />`
...
Voice Desk`
  ...
  function buildWhatsAppTemplate(contact: Contact): string {
    if (isVoiceDeskHotelContact(contact)) return `Hai visto cosa puoi fare nel tuo hotel?
${voiceDeskHotelVideoLine()}
${voiceDeskCallLine(contact)}`
    const person = contactDisplayName(contact)
    const business = contactBusinessName(contact)
