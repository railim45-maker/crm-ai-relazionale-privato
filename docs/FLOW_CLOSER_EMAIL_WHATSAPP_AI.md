...
## Collegamento diretto dei canali

Il livello operativo attuale è **assistito e manuale**. Il pulsante email usa `mailto:` per aprire il client email configurato nel dispositivo con destinatario, oggetto e corpo già compilati. Il pulsante WhatsApp usa `https://wa.me/` per aprire WhatsApp Web o l'app installata con il messaggio già compilato. In entrambi i casi l'utente controlla e conferma l'invio.

| Livello | Cosa fa | Costo | Rischio | Stato |
|---|---|---:|---|---|
| Assistito local-first | Apre email/WhatsApp con testo pronto | Zero | Basso, invio manuale | Implementato |
| Fallback dati mancanti | Copia il testo se mancano email o telefono | Zero | Basso | Implementato |
| Invio email automatico | Invia tramite SMTP/Gmail API | Variabile | Medio, richiede credenziali e consenso | Futuro |
| WhatsApp Business API | Invia e riceve messaggi via API ufficiale | Variabile | Medio/alto, richiede account business e template approvati | Futuro |

Per il CRM privato gratuito conviene mantenere il primo livello: permette di lavorare subito con messaggi AI selezionati, senza gestire credenziali sensibili e senza automatizzare comunicazioni che devono restare personalizzate.
