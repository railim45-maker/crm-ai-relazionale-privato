...
type OperatingArea = 'VoiceDesk Strutture' | 'NetFree/LCR' | 'Rubrica personale' | 'Trasversale'
...
const networkMaterials: NetworkMaterial[] = [
  {
    ...
    message: 'Messaggio rapido consigliato: “Hai visto cosa può fare VoiceDesk per una struttura come la tua? Ti mando un video breve; se ti riconosci nel problema, puoi chiedere info su voicedesk.it e provare direttamente la demo.”',
    consultantUse: 'Priorità VoiceDesk: apri con messaggio rapido + video, poi invito all’azione verso voicedesk.it per informazioni, approfondimenti e prova personale della demo. Usa una sola domanda sul problema operativo.',
    ...
    nextStep: 'Inviare video, link voicedesk.it e creare follow-up entro 48 ore per proporre prova demo o approfondimento.'
  },
  {
    id: 'blotix',
    area: 'Blotix',
...
const emptyDraft: ContactDraft = {
  ...
  sourceBatch: '',
  operatingArea: 'VoiceDesk Strutture',
  crossProposalNote: '',
  paidShareCapital: 0,
  realEstateValue: 0,
  inventoryValue: 0,
...
function inferOperatingArea(contact?: Partial<Contact> | null): OperatingArea { const explicit = contact?.operatingArea; if (explicit === 'VoiceDesk Strutture' || explicit === 'NetFree/LCR' || explicit === 'Rubrica personale' || explicit === 'Trasversale') return explicit; const haystack = [contact?.sourceBatch, contact?.category, contact?.subcategory, contact?.role, contact?.company, contact?.notes, contact?.services, contact?.messageAngle, contact?.personalizationHook, contact?.topics?.join(' ')].filter(Boolean).join(' ').toLowerCase(); if (contact?.netFreeStage && contact.netFreeStage !== 'Non avviato') return 'NetFree/LCR'; if (contact?.netFreeCandidateReason || contact?.netFreeLeadProfile || contact?.netFreeBriefForClaudio || haystack.includes('netfree') || haystack.includes('lcr')) return 'NetFree/LCR'; if (contact?.voiceDeskSegment || haystack.includes('voicedesk') || /hotel|albergo|b&b|bed and breakfast|ospitalit|reception|camere|booking|resort|residence|agriturismo|affittacamere|casa vacanze|campeggio|camping|glamping|villaggio turistico|ristorante|trattoria|pizzeria|osteria|prenotazioni/.test(haystack)) return 'VoiceDesk Strutture'; return 'Rubrica personale' }
function isNetFreeArea(contact?: Partial<Contact> | null) { return inferOperatingArea(contact) === 'NetFree/LCR' }
function isVoiceDeskArea(contact?: Partial<Contact> | null) { return inferOperatingArea(contact) === 'VoiceDesk Strutture' }
function isCrossProposal(contact?: Partial<Contact> | null) { return inferOperatingArea(contact) === 'Trasversale' || Boolean(contact?.crossProposalNote) }
...
function buildVoiceDeskSocialTemplate(contact: Contact, channel: Channel = 'Instagram') {
  ...
  if (profile.segment === 'Hotel') return `Hai visto cosa può fare VoiceDesk per una struttura come ${business}?\n${voiceDeskHotelVideoLine()}\n\nSe ti riconosci nel problema, puoi contattare VoiceDesk su ${voiceDeskDemoUrl} per informazioni, approfondimenti e per provare in prima persona la demo.`
  return `Buongiorno ${person}, ti mando VoiceDesk perché può aiutare ${business} a gestire chiamate e richieste ripetitive. Per informazioni, approfondimenti e prova diretta della demo: ${voiceDeskDemoUrl}`
}
...
function buildVoiceDeskEmailTemplate(contact: Contact) {
  ...
  const person = (contact.decisionMakerName || contact.name || 'buongiorno').trim()
  const business = (contact.company || contact.name || 'la sua struttura').trim()
  const subject = profile.segment === 'Hotel' ? 'Video rapido VoiceDesk per la tua struttura' : 'Video e demo VoiceDesk da provare'
  const hotelBody = `Buongiorno ${person},

messaggio rapidissimo: hai visto cosa può fare VoiceDesk per una struttura ricettiva come ${business}?

Ti mando un video breve: mostra come un assistente vocale AI può gestire prime richieste, informazioni ricorrenti e momenti in cui lo staff non può rispondere subito, senza togliere controllo alla struttura.

${voiceDeskHotelVideoLine()}

Se ti riconosci nel problema, puoi contattare VoiceDesk da qui per informazioni e approfondimenti:
${voiceDeskDemoUrl}

Da lì puoi anche provare in prima persona la demo e capire se ha senso fare un approfondimento di 10 minuti.

A presto,
VoiceDesk`
  const defaultBody = `Buongiorno ${person},

messaggio rapidissimo: ti mando VoiceDesk perché può aiutare attività come ${business} a gestire meglio chiamate, richieste ripetitive e primi contatti con i clienti.

L’idea non è sostituire le persone, ma recuperare contatti, dare risposte iniziali ordinate e lasciare allo staff ciò che richiede attenzione umana.

Per informazioni, approfondimenti e per provare in prima persona la demo:
${voiceDeskDemoUrl}

Se vuoi provare anche la demo telefonica rapida:
...
VoiceDesk`
  ...
  const html = profile.segment === 'Hotel'
    ? `<p>Buongiorno ${person},</p><p><strong>Messaggio rapidissimo:</strong> hai visto cosa può fare VoiceDesk per una struttura ricettiva come <strong>${business}</strong>?</p><p>Ti mando un video breve: mostra come un assistente vocale AI può gestire prime richieste, informazioni ricorrenti e momenti in cui lo staff non può rispondere subito, senza togliere controllo alla struttura.</p><p><a href="${voiceDeskHotelVideoLink()}" style="display:inline-block;background:#111827;color:#ffffff;padding:12px 18px;border-radius:12px;text-decoration:none;font-weight:700">Guarda il video esempio</a></p><p>Se ti riconosci nel problema, puoi contattare VoiceDesk per informazioni, approfondimenti e prova diretta della demo.</p><p><a href="${demoLink}" style="display:inline-block;background:#2563eb;color:#ffffff;padding:12px 18px;border-radius:12px;text-decoration:none;font-weight:700">Contatta VoiceDesk e prova la demo</a></p><p style="font-size:13px;color:#6b7280">Sito: ${voiceDeskDemoUrl}</p><img src="${openPixel}" width="1" height="1" alt="" style="display:none" />`
    : `<p>Buongiorno ${person},</p><p><strong>Messaggio rapidissimo:</strong> ti mando VoiceDesk perché può aiutare attività come <strong>${business}</strong> a gestire meglio chiamate, richieste ripetitive e primi contatti con i clienti.</p><p>L’idea non è sostituire le persone, ma recuperare contatti, dare risposte iniziali ordinate e lasciare allo staff ciò che richiede attenzione umana.</p><p><a href="${demoLink}" style="display:inline-block;background:#2563eb;color:#ffffff;padding:12px 18px;border-radius:12px;text-decoration:none;font-weight:700">Info, approfondimenti e demo VoiceDesk</a></p><p><a href="${callLink}" style="display:inline-block;background:#111827;color:#ffffff;padding:12px 18px;border-radius:12px;text-decoration:none;font-weight:700">Prova demo telefonica</a></p><p style="font-size:14px;color:#374151">Numero diretto: <strong>${profile.displayPhone}</strong></p><img src="${openPixel}" width="1" height="1" alt="" style="display:none" />`
...
function agentAnswer(question: string, contacts: Contact[], tasks: Task[], conversations: ConversationRecord[]) {
  ...
  if (contacts.length === 0) return 'Non hai ancora caricato contatti. Parti dal pulsante “Carica VoiceDesk 100” per la lavorazione aziende oppure “Carica NetFree 2914 → persone unite” per il flusso LCR/NetFree; poi completa email, sito e decision maker dei lead selezionati.'
  ...
  if (lower.includes('100') || lower.includes('database')) return `Database attuale: ${contacts.length} lead NetFree. A-list: ${aList.length}. Da qualificare: ${toQualify.length}. Il lavoro corretto è completare i campi mancanti dei lead A prima di aggiungere troppi nominativi.`
  ...
  return `Sintesi operativa: ${contacts.length} lead NetFree, ${aList.length} A-list, ${openTasks.length} task aperti. La mossa più utile ora è lavorare su ${hot[0].name}: ${hot[0].nextAction || hot[0].messageAngle || 'definisci il prossimo contatto'}.`
}
...
export default function DemoAppPage() {
  ...
  function isNetFreeContact(contact: Contact) {
    return isNetFreeArea(contact)
  }

  function isVoiceDeskContact(contact: Contact) {
    return isVoiceDeskArea(contact)
  }
  ...
  const voiceDeskContacts = useMemo(() => contacts.filter(isVoiceDeskContact), [contacts])
  const crossProposalContacts = useMemo(() => contacts.filter(isCrossProposal), [contacts])
  const rubricaPersonaleContacts = useMemo(() => contacts.filter((contact) => inferOperatingArea(contact) === 'Rubrica personale'), [contacts])
  const filteredContacts = useMemo(() => {
    const q = search.toLowerCase().trim()
    return q ? voiceDeskContacts.filter((c) => [c.name, c.company, c.role, c.email, c.phone, c.notes, c.topics.join(' '), c.category, c.subcategory, c.city, c.website, c.socialFacebookUrl, c.socialInstagramUrl, c.confirmedFacebookPage, c.decisionMakerName, c.personalizationHook, c.messageAngle, c.outreachStage, c.priorityLevel, c.operatingArea, c.crossProposalNote].join(' ').toLowerCase().includes(q)) : voiceDeskContacts
  }, [voiceDeskContacts, search])
  ...
  const mailingContacts = useMemo(() => voiceDeskContacts.filter((contact) => {
    ...
    return true
  }), [voiceDeskContacts, mailingPriority, mailingStage])
  ...
  const excludedEmailCount = voiceDeskContacts.filter((contact) => !isValidEmail(contactEmail(contact))).length
  const voiceDeskStats = useMemo(() => voiceDeskContacts.reduce((acc, contact) => {
    ...
    return acc
  }, { sent: 0, open: 0, demo: 0, call: 0 }), [voiceDeskContacts])
  ...
  function draftFromContact(contact: Contact): ContactDraft {
    return {
      ...
      operatingArea: contact.operatingArea || inferOperatingArea(contact),
      crossProposalNote: contact.crossProposalNote || '',
  ...
  function contactFromDraft(timestamp: string): Omit<Contact, 'id' | 'createdAt' | 'updatedAt' | 'lastContact'> {
    return {
      ...
      operatingArea: draft.operatingArea || inferOperatingArea(draft),
      crossProposalNote: draft.crossProposalNote?.trim(),
  ...
  function createLeadFromQuickPaste(targetStatus: ContactStatus = 'Lead', operatingArea: OperatingArea = 'VoiceDesk Strutture') {
    ...
    const newContact: Contact = normalizeContact({
      ...
      sourceBatch: parsed.sourceBatch || (operatingArea === 'NetFree/LCR' ? 'Incolla rapido NetFree/LCR' : operatingArea === 'VoiceDesk Strutture' ? 'Incolla rapido VoiceDesk strutture' : 'Incolla rapido'),
      operatingArea,
      crossProposalNote: operatingArea === 'Trasversale' ? 'Lead trasversale: valutare separatamente quale proposta aprire per prima.' : parsed.crossProposalNote || '',
      netFreeStage: operatingArea === 'NetFree/LCR' ? (parsed.netFreeStage || 'Lead raccolto') : parsed.netFreeStage,
    } as Contact)
    ...
    setQuickPasteText('')
    setSection('contacts')
    setTemporaryQuickPasteFeedback(`${targetStatus === 'Cliente' ? 'Cliente' : 'Lead'} salvato in ${operatingArea}: ${newContact.name}`)
  }
...
${voiceDeskHotelVideoLine()}`
  ...
  function buildVoiceDeskRapidMessage(contact?: Contact | null) {
    const business = contact ? contactBusinessName(contact) : 'la tua struttura'
    const video = voiceDeskHotelVideoLink()
    return `Hai visto cosa può fare VoiceDesk per una struttura come ${business}? Ti mando un video breve: ${video}\n\nSe ti riconosci nel problema, puoi contattare VoiceDesk su ${voiceDeskDemoUrl} per informazioni, approfondimenti e per provare in prima persona la demo.`
  }

  function buildNetFreeRapidMessage(contact?: Contact | null) {
    const person = contact ? contactDisplayName(contact) : 'ciao'
    return `${person}, ti condivido un’informazione su NetFree/LCR: è un percorso separato da VoiceDesk, pensato per persone che vogliono capire se i propri asset o relazioni possono generare valore e reddito passivo in modo guidato.\n\nIl primo passo non è aderire subito, ma capire profilo, asset disponibili, disponibilità e coerenza con il percorso. Se ti interessa, fissiamo un approfondimento breve e senza pressione.`
  }

  function copyMessagingGuide(kind: 'voicedesk' | 'netfree') {
    const text = kind === 'voicedesk' ? buildVoiceDeskRapidMessage(selectedContact) : buildNetFreeRapidMessage(selectedContact)
    navigator.clipboard.writeText(text).then(() => setTemporaryAssistedFeedback(kind === 'voicedesk' ? 'Messaggio rapido VoiceDesk copiato' : 'Messaggio NetFree/LCR copiato')).catch(() => window.alert('Non sono riuscito a copiare automaticamente. Seleziona il testo e copialo manualmente.'))
  }
...
Esito da registrare: Approvato, Bocciato o Da valutare. Se approvato, la fase PEF diventa un task interno da gestire manualmente con un tecnico, senza passaggi automatici non presidiati.`
  ...
  function processNetFreeImportRaw(rawInput: string, sourceLabel = 'file selezionato') {
    ...
    const existing = new Map(contacts.filter(isNetFreeContact).map((contact) => [netFreeDedupeKey(contact), contact]))
    const fresh: Contact[] = []
  ...
  const stageGroups = outreachStages.map((stage) => ({
    stage,
    items: voiceDeskContacts.filter((contact) => (contact.outreachStage || 'Da qualificare') === stage),
  }))
  const netFreeContacts = contacts.filter(isNetFreeContact)
  const selectedNetFreeContact = netFreeContacts.find((contact) => contact.id === selectedContactId) || netFreeContacts[0]
  ...
  <div className="flex flex-col lg:flex-row lg:items-start gap-5">
    ...
    <div className="flex-1 space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        <button onClick={() => createLeadFromQuickPaste('Lead', 'VoiceDesk Strutture')} disabled={!quickPasteText.trim()} className="rounded-2xl bg-blue-700 text-white px-4 py-3 font-semibold hover:bg-blue-800 disabled:opacity-40"><Plus className="w-4 h-4 inline mr-2" />Crea lead VoiceDesk</button>
        <button onClick={() => createLeadFromQuickPaste('Lead', 'NetFree/LCR')} disabled={!quickPasteText.trim()} className="rounded-2xl bg-teal-700 text-white px-4 py-3 font-semibold hover:bg-teal-800 disabled:opacity-40"><Phone className="w-4 h-4 inline mr-2" />Crea lead NetFree/LCR</button>
        <button onClick={() => createLeadFromQuickPaste('Cliente', 'Rubrica personale')} disabled={!quickPasteText.trim()} className="rounded-2xl bg-green-700 text-white px-4 py-3 font-semibold hover:bg-green-800 disabled:opacity-40"><Save className="w-4 h-4 inline mr-2" />Crea cliente rubrica</button>
      ...
      <div className="grid md:grid-cols-3 gap-3 text-xs text-gray-600">
        ...
        <div className="rounded-2xl bg-stone-50 border p-3"><strong>Ambiente obbligatorio</strong><br />VoiceDesk e NetFree/LCR non partono dallo stesso messaggio: scegli prima il percorso corretto, poi eventualmente marca la scheda come trasversale.</div>
      </div>
    </div>
  </div>
</section>
